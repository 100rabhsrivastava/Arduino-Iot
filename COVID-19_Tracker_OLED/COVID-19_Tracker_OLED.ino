 ...
 */
 ...
 #include "images.h"  // Include custom images
  ...
  const char* ssid = "DIR-615-A612";              //WIFI SSID Name                             
  const char* password =  "12345678";        //WIFI Password
  const char* host = "api.thingspeak.com";  //We read the data from this host                                   
  const int httpPortRead = 80;                                           
                                                  
  const char* url1 = "/apps/thinghttp/send_request?api_key=HU0YMOFZKBTTW7GL";     //Change this URL Cases
  const char* url2 = "/apps/thinghttp/send_request?api_key=FY3LBR8F7U1WZ7TB";   //Deaths
  const char* url3 = "/apps/thinghttp/send_request?api_key=ZAIKJ1ZQW7965K3Q";   //Recovered
  ...
  void drawFrame2(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
    ...
    display->drawString(64, 35, "WORLD LEVEL");   //Change Country name
   }
